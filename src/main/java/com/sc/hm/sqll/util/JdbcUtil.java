...
public class JdbcUtil {
    ...
    private static final String POSTGRES_URL = "jdbc:postgresql://{0}:{1}/{2}";
    private static final String SYSBASE_URL = "jdbc:sybase:Tds:{0}:{1}/{2}";
    private static final String DERBY_URL = "jdbc:derby://{0}:{1}/{2}";

    /**
     * Test to see if the database configuration is valid. This is done by
     * obtaining a valid connection to the underlying db.
     *
     * @param config
     */
    ...
    public static Connection getConnection(ConnectionConfig config)
        ...
        if (DatabaseType.ORACLE == config.getDbType()) {
            ...
            urlTemplate = DB2_URL;
            sidOrService = config.getSid();
        } else if (DatabaseType.POSTGRES == config.getDbType()) {
            urlTemplate = POSTGRES_URL;
            sidOrService = config.getSid();
